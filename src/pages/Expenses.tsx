...
import { formatTZS, transactions as mockTxs, Transaction } from '@/lib/mock-data';
import { motion, AnimatePresence } from 'framer-motion';
import { Plus, ArrowUpRight, ArrowDownRight, X, Filter } from 'lucide-react';
...
const categoryColors: Record<string, string> = {
  ...
  Education: 'hsl(162,63%,40%)',
  Business: 'hsl(190,60%,45%)',
  Other: 'hsl(0,0%,60%)',
};
...
const Expenses = () => {
  const { t } = useI18n();
  const [txs, setTxs] = useState<Transaction[]>(mockTxs);
  ...
  const catData = categories.map(cat => ({
    name: cat,
    value: expenses.filter(tx => tx.category === cat).reduce((s, tx) => s + tx.amount, 0),
  })).filter(c => c.value > 0);
            ...
            <ResponsiveContainer width="50%" height="100%">
              <PieChart>
                <Pie data={catData} cx="50%" cy="50%" innerRadius={40} outerRadius={65} dataKey="value" strokeWidth={2} stroke="hsl(var(--card))">
                  {catData.map((entry, i) => (
                    <Cell key={i} fill={categoryColors[entry.name] || '#888'} />
                  ))}
                </Pie>
      ...
      <AnimatePresence>
        {showAdd && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-foreground/30 glass flex items-end justify-center"
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md rounded-t-2xl bg-card p-5 safe-bottom"
            ...
            >
              ...
              <div className="flex gap-2 mb-4">
                {(['expense', 'income'] as const).map(tp => (
                  <button
                    key={tp}
                    onClick={() => setNewTx(p => ({ ...p, type: tp }))}
                    className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors ${
                      newTx.type === tp
                        ? tp === 'expense' ? 'bg-destructive text-destructive-foreground' : 'bg-success text-success-foreground'
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-input bg-background px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                />
                <Button onClick={handleAdd} className="w-full gradient-primary border-0 text-primary-foreground rounded-xl py-3">
                  {t('gen.save')}
                </Button>
              </div>
            </motion.div>
