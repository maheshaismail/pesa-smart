    ...
    <div className="space-y-5 pb-24 pt-2">
      ...
      {monthlyChart.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl bg-card p-4 shadow-card">
          <h3 className="text-sm font-semibold font-display mb-3">Monthly Overview</h3>
          <div className="h-48">
      ...
      {monthlyChart.length > 1 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="rounded-xl bg-card p-4 shadow-card">
          <h3 className="text-sm font-semibold font-display mb-3">Net Savings Trend</h3>
          <div className="h-36">
            <ResponsiveContainer width="100%" height="100%">
      ...
      {categoryData.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-xl bg-card p-4 shadow-card">
          <h3 className="text-sm font-semibold font-display mb-3">Spending by Category</h3>
      ...
      {weeklyData.length > 1 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="rounded-xl bg-card p-4 shadow-card">
          <h3 className="text-sm font-semibold font-display mb-3">Weekly Spending</h3>
          <div className="h-36">
            <ResponsiveContainer width="100%" height="100%">
      ...
      {dayAvgData.some(d => d.avg > 0) && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="rounded-xl bg-card p-4 shadow-card">
          <h3 className="text-sm font-semibold font-display mb-3 flex items-center gap-1.5"><Calendar size={14} className="text-primary" /> Avg Spending by Day</h3>
          <div className="h-28">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="rounded-xl gradient-primary p-4 shadow-elevated">
        <h3 className="text-sm font-bold font-display text-primary-foreground flex items-center gap-1.5 mb-3">
          <Brain size={16} /> AI Financial Predictions
        </h3>

        {loadingPredictions ? (
          <div className="flex items-center justify-center py-6">
            <Loader2 size={20} className="animate-spin text-primary-foreground/70" />
            <span className="text-xs text-primary-foreground/70 ml-2">Analyzing your data...</span>
          </div>
        ) : predictions ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 bg-primary-foreground/10 rounded-lg p-3">
              <div className="flex items-center gap-1.5 bg-primary-foreground/20 rounded-md px-2 py-1">
                {trendIcon}
                <span className="text-[10px] text-primary-foreground font-medium capitalize">{predictions.trend}</span>
              </div>
              <div>
                <p className="text-[10px] text-primary-foreground/70">Predicted Next Month</p>
            ...
            {predictions.forecast && (
              <div className="space-y-1">
                <p className="text-[10px] text-primary-foreground/70 font-medium">Category Forecast</p>
            ...
            {predictions.anomalies?.length > 0 && (
              <div className="space-y-1">
                <p className="text-[10px] text-primary-foreground/70 font-medium flex items-center gap-1"><AlertTriangle size={10} /> Anomalies Detected</p>
          ...
          <div className="text-center py-4">
            <p className="text-xs text-primary-foreground/70 mb-2">Add at least 5 transactions to unlock AI predictions</p>
            <Button onClick={fetchPredictions} disabled={loadingPredictions} size="sm" variant="secondary" className="text-xs rounded-lg">
              {loadingPredictions ? 'Loading...' : 'Generate Predictions'}
            </Button>
          </div>
        )}
      </motion.div>
      ...
      {filtered.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-8">No transaction data for this period. Add transactions to see reports!</p>
      )}
    </div>
  );
};
